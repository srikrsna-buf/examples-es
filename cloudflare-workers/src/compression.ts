...
function createCompression(name: 'gzip' | 'deflate' | 'deflate-raw'): Compression {
	return {
		name,
...
async function readAll(stream: ReadableStream, readMaxBytes: number | undefined) {
	...
	for (;;) {
		...
		if (readMaxBytes !== undefined && length > readMaxBytes) {
			throw new ConnectError(`message is larger than configured readMaxBytes ${readMaxBytes} after decompression`, Code.ResourceExhausted);
		}
		if (next.done) {
			break;
		}
